...
public:
  ...
  /// Add a local label to all callsites that don't already have a
  /// label. Valid only before CFG is constructed.
  /// Used for patching frametables generated by ocaml
  /// compiler for the garbage collector. Maybe a better way to do it
  /// would be to reuse exsiting frame annotations for c++ exception
  /// handling.
  void labelCallsites();
